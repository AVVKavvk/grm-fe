...
const CracoMerchandiseSection = () => {
  const merchandise = [
    ...
    },
    {
      ...
      sizes: ["15cm", "25cm"],
      new: true,
      description: "Adorable plush version of Craco - perfect for kids and collectors"
    },
    {
      id: 4,
    ...
    },
    {
      ...
      category: "Collectibles",
      bestseller: true,
      description: "Set of 10 waterproof stickers featuring Craco's expressions"
    }
  ];

  const benefits = [
    { icon: Package, title: "Premium Quality", desc: "High-quality materials and prints" },
    { icon: Truck, title: "Fast Shipping", desc: "Free shipping on orders above ₹500" },
    { icon: Star, title: "Limited Edition", desc: "Exclusive marathon merchandise" }
  ];
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-bold mb-6">
            Craco's 
            <span className="bg-gradient-sunset bg-clip-text text-transparent"> Adventure Store</span>
          </h2>
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto mb-8">
            Take home a piece of the adventure! Get exclusive Craco merchandise and show your marathon spirit.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {merchandise.slice(0, 3).map((item) => (
            <Card key={item.id} className="overflow-hidden hover:shadow-medium transition-smooth group">
              <div className="relative">
                <div className="bg-gradient-ocean p-8 text-center">
                  ...
                  <div className="absolute top-3 right-3 flex flex-col gap-1">
                    {item.bestseller && <Badge className="bg-yellow-500 text-yellow-50 text-xs">Bestseller</Badge>}
                    {item.limited && <Badge className="bg-red-500 text-red-50 text-xs">Limited</Badge>}
                    {item.new && <Badge className="bg-green-500 text-green-50 text-xs">New</Badge>}
                  </div>
                </div>
                    ...
                    {item.sizes && (
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-1">
                          {item.sizes.map((size) => (
                            <span key={size} className="text-xs bg-muted px-2 py-1 rounded">{size}</span>
                          ))}
                        </div>
                      </div>
                    )}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-1">
                          {item.colors.map((color) => (
                            <span key={color} className="text-xs bg-muted px-2 py-1 rounded">{color}</span>
                          ))}
                        </div>
                      </div>
                    )}
