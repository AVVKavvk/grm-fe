...
const RaceRouteSection = () => {
  const routeDetails = [
    ...
    },
    {
      ...
      image: villageImage,
      highlights: ["Colonial architecture", "Local communities", "Cultural immersion"]
    },
    {
  ...
  const courseSpecs = [
    { label: "Total Distance", value: "42.2 KM", icon: <Award className="w-4 h-4" /> },
    { label: "Start Time", value: "4:00 AM", icon: <Clock className="w-4 h-4" /> },
    { label: "Cut-off Time", value: "5 Hours", icon: <Clock className="w-4 h-4" /> },
    { label: "Elevation Gain", value: "150m", icon: <MapPin className="w-4 h-4" /> }
  ];
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">
            India's Most 
            <span className="bg-gradient-primary bg-clip-text text-transparent"> Scenic Route</span>
          </h2>
          ...
          {routeDetails.map((phase, index) => (
            <Card key={index} className="overflow-hidden hover:shadow-medium transition-all group">
              ...
              <CardContent className="p-4">
                ...
                <p className="text-primary text-xs font-semibold mb-1">{phase.location}</p>
                <p className="text-muted-foreground text-xs mb-3">{phase.description}</p>
                <div className="flex flex-wrap gap-1">
                  {phase.highlights.map((highlight, idx) => (
                    <span key={idx} className="text-xs bg-accent/10 text-accent px-1.5 py-0.5 rounded">
                      {highlight}
                    </span>
                  ))}
                </div>
